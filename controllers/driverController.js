...
exports.getAllBooking = async(req, res, next) => {
    try {
        ...
        const bookingInfo = await DriverPayment.find({driverId: driverId._id}).populate('car').populate({path:'bookingId', select: 'travelerInfo'});
        const booking = await Booking.find({ driver: driverId._id }).sort({ bookingDate: 1 });
        console.log(booking);
    ...
    try {
        if (req.body.id) {
            ...
            if (verify.length == 1) {
                ...
                if (dbdate == newDate) {
                    const update = await Booking.updateOne({ _id: req.body.id }, { travelStatus: 1 });
    ...
    try {
        ...
        if (checkBooking.travelStatus != 2) {
            if (checkBooking.dueAmount == 0) {
                const updateBooking = await Booking.updateOne({ _id: req.body.id }, { travelStatus: 2 });
                ...
                } else { return response(400, 0, 'Please enter due amount', res); }
            }
            console.log(checkPayment.dueAmount);
        } else { return response(400, 0, 'Something error! Booking not found or already completed', res); }
    } catch (err) {
        console.log(err)
        return response(400, 0, 'Something error', res);
    }
...
exports.getCurrentLocation = async(req, res, next) => {
    try {
        ...
        if (currentLocation.length < 1) return response(400, 0, 'no Record found', res);
...
exports.updateLocation = async(req, res, next) => {
    try {
        ...
        if (!req.body.address) return response(400, 0, 'Address or Driver id is required', res);
        const update = await Driver.updateOne({ userId: req.authUser.id }, {
            currentLocation: {
                date: new Date(),
                location: req.body.address
            }
        });
        if (!update) return response(400, 0, 'Something error! Location not updated', res);
...
const getCurrentLocation = async(req, res, next) => {
    try {
        const currentLocation = await Driver.findOne({ userId: req.authUser.id }).select('currentLocation');
    } catch (error) {
        console.log(error)
        return response(400, 0, error, res);
    }
